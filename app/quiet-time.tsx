...
function ActivityDetail({ mode, state, setPatch, onBack }: { mode: Mode; state: QuietState; setPatch: (p: Partial<QuietState>) => void; onBack: () => void }) { const ff = useFontFamily(); const [scrollEnabled, setScrollEnabled] = useState(true); return <ScrollView scrollEnabled={scrollEnabled} contentContainerStyle={styles.detail}><Pressable onPress={onBack} style={styles.done}><Text style={styles.doneText}>Done</Text></Pressable>{mode === 'colouring' ? <Colouring state={state} setPatch={setPatch} ff={ff} /> : mode === 'mahjong' ? <Mahjong state={state} setPatch={setPatch} ff={ff} /> : <Jigsaw state={state} setPatch={setPatch} ff={ff} setScrollEnabled={setScrollEnabled} />}</ScrollView>; }
